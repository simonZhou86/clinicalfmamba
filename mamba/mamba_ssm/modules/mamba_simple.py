...
class Mamba(nn.Module):
    ...
    def __init__(
        ...
        if bimamba_type in ["v0", "v3", "v5", "v7", "v9", "3dd3_fuse"]:
            self.in_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
            self.in_proj_b = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)
        elif bimamba_type == "v4":
            self.in_proj = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)
        ...
        # bidirectional
        if bimamba_type in ["v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "3dd3", "3dd3_fuse"]:
            A_b = repeat(
                torch.arange(1, self.d_state + 1, dtype=torch.float32, device=device),
                "n -> d n",
                d=self.d_inner,
        ...
        if bimamba_type in ["v2", "v3", "v4", "v6", "v7", "3dd3", "3dd3_fuse"]:
            self.conv1d_b = nn.Conv1d(
                in_channels=self.d_inner,
                out_channels=self.d_inner,
                bias=conv_bias,
        ...
        if bimamba_type in ["v6", "v7", "v8", "v9", "3dd3", "3dd3_fuse"]:
            A_c = repeat(
                torch.arange(1, self.d_state + 1, dtype=torch.float32, device=device),
                "n -> d n",
                d=self.d_inner,
            ...
            A_e_log = torch.log(A_e)  # Keep A_d_log in fp32
            ...
            A_f_log = torch.log(A_f)  # Keep A_d_log in fp32
            ...
            A_g_log = torch.log(A_g)  # Keep A_d_log in fp32
        ...
        if bimamba_type in ["3dd3", "3dd3_fuse"]:
            ...
            A_e_log = torch.log(A_e)  # Keep A_c_log in fp32
            self.A_e_log = nn.Parameter(A_e_log)
            self.A_e_log._no_weight_decay = True
            ...
            A_h_log = torch.log(A_h)  # Keep A_d_log in fp32
    ...
    def forward(self, hidden_states, extra_emb=None, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v0":
            ...
            elif self.bimamba_type == "3dd3":
                A_b = -torch.exp(self.A_b_log.float())
                A_c = -torch.exp(self.A_c_log.float())
                A_d = -torch.exp(self.A_d_log.float())
                A_e = -torch.exp(self.A_e_log.float())
                A_f = -torch.exp(self.A_f_log.float())
                # Direction 1: normal flatten
                xz_1 = xz

                # Direction 2: Down-Up (backward of xz_1)
                # Flip the sequence dimension
                xz_2 = xz.flip([-1])

                # Direction 3: Up down
                xz_3 = rearrange(xz, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                xz_3 = rearrange(xz_3.transpose(-2, -1).contiguous(), 
                                "b d w h z -> b d (w h z)",
                                w=self.input_w, h=self.input_h, z=self.input_z)

                # Direction 4: tr-scan row-wise
                # xz_4 = rearrange(xz, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                # xz_4 = rearrange(xz_4.transpose(-3, -1).contiguous(),
                #                 "b d w z h -> b d (w z h)",
                #                 w=self.input_w, h=self.input_h, z=self.input_z)

                # # Direction 5: tri-scan column wise
                # xz_5 = rearrange(xz, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                # xz_5 = rearrange(xz_5.transpose(-3, -1).contiguous(),
                #                 "b d z w h -> b d (z w h)",
                #                 w=self.input_w, h=self.input_h, z=self.input_z)

                # Apply Mamba to all 6 directions
                out_1 = mamba_inner_fn_no_out_proj(
                    xz_1,
                    self.conv1d.weight,
                    self.conv1d.bias,
                    self.x_proj.weight,
                ...
                out_2 = mamba_inner_fn_no_out_proj(
                    xz_2,
                    self.conv1d_b.weight,
                    self.conv1d_b.bias,
                    self.x_proj_b.weight,
                ...
                out_3 = mamba_inner_fn_no_out_proj(
                    xz_3,
                    self.conv1d_c.weight,
                    self.conv1d_c.bias,
                    self.x_proj_c.weight,
                ...
                # out_4 = mamba_inner_fn_no_out_proj(
                #     xz_4,
                #     self.conv1d_d.weight,
                #     self.conv1d_d.bias,
                #     self.x_proj_d.weight,
                ...
                # out_5 = mamba_inner_fn_no_out_proj(
                #     xz_5,
                #     self.conv1d_e.weight,
                #     self.conv1d_e.bias,
                #     self.x_proj_e.weight,
                #     self.dt_proj_e.weight,
                #     A_e,
                #     None,
                #     None,
                #     self.D_e.float(),
                ...
                # Restore original spatial order for each direction
                # Direction 1: No change needed
                out_1_final = out_1

                # Direction 2: Flip back
                out_2_final = out_2.flip([-1])

                # Direction 3: Restore from updown
                out_3_temp = rearrange(out_3, "b d (w h z) -> b d w h z", h=self.input_h, w=self.input_w, z=self.input_z)
                out_3_final = rearrange(out_3_temp.transpose(-2, -1).contiguous(), 
                                "b d h w z -> b d (h w z)",
                                w=self.input_w, h=self.input_h, z=self.input_z)

                # Direction 4: Restore from tri-scan row-wise
                # out_4_temp = rearrange(out_4, "b d (w z h) -> b d w z h", h=self.input_h, w=self.input_w, z=self.input_z)
                # out_4_final = rearrange(out_4_temp.transpose(-3, -1).contiguous(),
                #                 "b d h w z -> b d (h w z)",
                #                 w=self.input_w, h=self.input_h, z=self.input_z)

                # # Direction 5: Restore from tri-scan column-wise
                # out_5_temp = rearrange(out_5, "b d (z w h) -> b d z w h", h=self.input_h, w=self.input_w, z=self.input_z)
                # out_5_final = rearrange(out_5_temp.transpose(-3, -1).contiguous(),
                #                 "b d h w z -> b d (h w z)",
                #                 w=self.input_w, h=self.input_h, z=self.input_z)
                
                # out_4_final + out_5_final
                if not self.if_devide_out:
                    if self.use_norm:
                        out = F.linear(self.norm(rearrange(out_1_final + out_2_final.flip([-1]) + out_3_final, "b d l -> b l d")),
                                       self.out_proj.weight, self.out_proj.bias)
                    else:
                        out = F.linear(rearrange(out_1_final + out_2_final.flip([-1]) + out_3_final, "b d l -> b l d"),
                                       self.out_proj.weight, self.out_proj.bias)
                else:
                    if self.use_norm:
                        out = F.linear(self.norm(rearrange(out_1_final + out_2_final.flip([-1]) + out_3_final, "b d l -> b l d") / 5),
                                       self.out_proj.weight, self.out_proj.bias)
                    else:
                        out = F.linear(rearrange(out_1_final + out_2_final.flip([-1]) + out_3_final, "b d l -> b l d") / 5,
                                       self.out_proj.weight, self.out_proj.bias)
            
            
            elif self.bimamba_type == "3dd3_fuse":
                # NOTE: Backword scan is optional
                x, z = xz.chunk(2, dim=1)
                x_b, z_b = xz.flip([-1]).chunk(2, dim=1)
                extra = extra_emb
                extra_b = extra_emb.flip([-1])

                # Height-Width transpose scan (same as your code)
                xz_b = rearrange(xz, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                xz_b = rearrange(xz_b.transpose(-3, -2).contiguous(),  # transpose h and w
                                "b d w h z -> b d (w h z)",
                                w=self.input_w, h=self.input_h, z=self.input_z)

                x_c, z_c = xz_b.chunk(2, dim=1)
                x_d, z_d = xz_b.flip([-1]).chunk(2, dim=1)
                extra_emb_b = rearrange(extra_emb, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                extra_emb_b = rearrange(extra_emb_b.transpose(-3, -2).contiguous(),
                                        "b d w h z -> b d (w h z)",
                                        w=self.input_w, h=self.input_h, z=self.input_z)
                extra_c = extra_emb_b
                extra_d = extra_emb_b.flip([-1])

                # Height-Depth transpose scan
                xz_c = rearrange(xz, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                xz_c = rearrange(xz_c.transpose(-3, -1).contiguous(),  # transpose h and z
                                "b d z w h -> b d (z w h)",
                                z=self.input_z, w=self.input_w, h=self.input_h)

                x_e, z_e = xz_c.chunk(2, dim=1)
                x_f, z_f = xz_c.flip([-1]).chunk(2, dim=1)
                extra_emb_c = rearrange(extra_emb, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                extra_emb_c = rearrange(extra_emb_c.transpose(-3, -1).contiguous(),
                                        "b d z w h -> b d (z w h)",
                                        z=self.input_z, w=self.input_w, h=self.input_h)
                extra_e = extra_emb_c
                extra_f = extra_emb_c.flip([-1])

                Width-Depth transpose scan
                xz_d = rearrange(xz, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                xz_d = rearrange(xz_d.transpose(-2, -1).contiguous(),  # transpose w and z
                                "b d h z w -> b d (h z w)",
                                h=self.input_h, z=self.input_z, w=self.input_w)

                x_g, z_g = xz_d.chunk(2, dim=1)
                x_h, z_h = xz_d.flip([-1]).chunk(2, dim=1)
                extra_emb_d = rearrange(extra_emb, "b d (h w z) -> b d h w z", h=self.input_h, w=self.input_w, z=self.input_z)
                extra_emb_d = rearrange(extra_emb_d.transpose(-2, -1).contiguous(),
                                        "b d h z w -> b d (h z w)",
                                        h=self.input_h, z=self.input_z, w=self.input_w)
                extra_g = extra_emb_d
                ...
                # Direction 3: H-W transpose forward scan (w→h→z order)
                A_c = -torch.exp(self.A_c_log.float())
                if causal_conv1d_fn is None:
                    x_c = self.act(self.conv1d_c(x_c)[..., :seqlen])
                else:
                ...
                # Direction 4: H-W transpose backward scan
                A_d = -torch.exp(self.A_d_log.float())
                if causal_conv1d_fn is None:
                    x_d = self.act(self.conv1d_d(x_d)[..., :seqlen])
                else:
                ...
                # Direction 5: H-Z transpose forward scan (z→w→h order)
                A_e = -torch.exp(self.A_e_log.float())
                if causal_conv1d_fn is None:
                ...
                # Direction 6: H-Z transpose backward scan
                A_f = -torch.exp(self.A_f_log.float())
                if causal_conv1d_fn is None:
                ...
                # Direction 7: W-Z transpose forward scan (h→z→w order)
                ...
                # Direction 8: W-Z transpose backward scan
                ...
                # Combination: Restore scan directions to original 3D layout
                # H-W transpose scans (directions c,d): w→h→z back to h→w→z
                y_c = rearrange(y_c, "b d (w h z) -> b d w h z", w=self.input_w, h=self.input_h, z=self.input_z)
                y_c = rearrange(y_c.transpose(-3, -2).contiguous(),  # w,h,z -> h,w,z
                                "b d h w z -> b d (h w z)",
                                h=self.input_h, w=self.input_w, z=self.input_z)
                y_d = rearrange(y_d.flip([-1]), "b d (w h z) -> b d w h z", w=self.input_w, h=self.input_h, z=self.input_z)
                y_d = rearrange(y_d.transpose(-3, -2).contiguous(),  # w,h,z -> h,w,z
                                "b d h w z -> b d (h w z)",
                                h=self.input_h, w=self.input_w, z=self.input_z)

                # H-Z transpose scans (directions e,f): z→w→h back to h→w→z
                y_e = rearrange(y_e, "b d (z w h) -> b d z w h", z=self.input_z, w=self.input_w, h=self.input_h)
                y_e = rearrange(y_e.transpose(-3, -1).contiguous(),  # z,w,h -> h,w,z
                                "b d h w z -> b d (h w z)",
                                h=self.input_h, w=self.input_w, z=self.input_z)
                y_f = rearrange(y_f.flip([-1]), "b d (z w h) -> b d z w h", z=self.input_z, w=self.input_w, h=self.input_h)
                y_f = rearrange(y_f.transpose(-3, -1).contiguous(),  # z,w,h -> h,w,z
                                "b d h w z -> b d (h w z)",
                                h=self.input_h, w=self.input_w, z=self.input_z)

                # W-Z transpose scans (directions g,h): h→z→w back to h→w→z
                y_g = rearrange(y_g, "b d (h z w) -> b d h z w", h=self.input_h, z=self.input_z, w=self.input_w)
                y_g = rearrange(y_g.transpose(-2, -1).contiguous(),  # h,z,w -> h,w,z
                                "b d h w z -> b d (h w z)",
                                h=self.input_h, w=self.input_w, z=self.input_z)
                y_h = rearrange(y_h.flip([-1]), "b d (h z w) -> b d h z w", h=self.input_h, z=self.input_z, w=self.input_w)
                y_h = rearrange(y_h.transpose(-2, -1).contiguous(),  # h,z,w -> h,w,z
                                "b d h w z -> b d (h w z)",
                                h=self.input_h, w=self.input_w, z=self.input_z)

                # Final combination of all 8 directions
                # y_g + y_h
                if not self.if_devide_out:
                    if self.use_norm:
                        out = F.linear(self.norm(rearrange(y + y_b.flip([-1]) + y_c + y_d + y_e + y_f + y_g + y_h, "b d l -> b l d")),
                                       self.out_proj.weight, self.out_proj.bias)
                    else:
                        out = F.linear(rearrange(y + y_b.flip([-1]) + y_c + y_d + y_e + y_f + y_g + y_h, "b d l -> b l d"),
                                       self.out_proj.weight, self.out_proj.bias)
                else:
                    if self.use_norm:
                        out = F.linear(self.norm(rearrange(y + y_b.flip([-1]) + y_c + y_d + y_e + y_f + y_g + y_h, "b d l -> b l d") / 8),
                                       self.out_proj.weight, self.out_proj.bias)
                    else:
                        out = F.linear(rearrange(y + y_b.flip([-1]) + y_c + y_d + y_e + y_f + y_g + y_h, "b d l -> b l d") / 8,
                                       self.out_proj.weight, self.out_proj.bias)
