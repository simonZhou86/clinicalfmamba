...
def get_common_file(target_dir):
    ...
    '''
    ct = os.path.join(target_dir, "SPECT") # CT
...
def get_loader_seperate(ct, mri, tv_ratio, bs):
    '''
    ct: ct data
    mri: mri data
    ...
    total_len_ct = ct.shape[0]
    total_len_mri = mri.shape[0]
    if total_len_ct != total_len_mri:
        raise ValueError("CT and MRI datasets must have the same length")

    n_train_ct = int(tv_ratio * total_len_ct)
    n_train_mri = int(tv_ratio * total_len_mri)

    # get the index for both ct and mri, return loaders for both
    train_set_ct, val_set_ct = random_split(getIndex(total_len_ct), lengths=(n_train_ct, total_len_ct - n_train_ct))
    train_loader_ct = DataLoader(train_set_ct, batch_size=bs, num_workers=0, shuffle=True, drop_last=False)
    val_loader_ct = DataLoader(val_set_ct, batch_size=bs, num_workers=0, shuffle=False, drop_last=False)

    # train_set_mri, val_set_mri = random_split(getIndex(total_len_mri), lengths=(n_train_mri, total_len_mri - n_train_mri))
    train_loader_mri = DataLoader(train_set_ct, batch_size=bs, num_workers=0, shuffle=True, drop_last=False)
    val_loader_mri = DataLoader(val_set_ct, batch_size=bs, num_workers=0, shuffle=False, drop_last=False)

    return (train_loader_ct, val_loader_ct), (train_loader_mri, val_loader_mri)
...
def get_cv_dataset(n, fold, shuffle=False):
    X = np.arange(n)
    kfold = KFold(n_splits=fold, shuffle=shuffle)  ## kfold为KFolf类的一个对象
    return_res = []
    for a, b in kfold.split(X):
        fold_train = []
        fold_val = []
